...
std::array<uint16_t, 256> colorPalette;

void setupColorPalette()
{
  colorPalette.fill(HX8357_BLACK);
  colorPalette.at(1) = HX8357_RED;
  colorPalette.at(2) = HX8357_GREEN;
  colorPalette.at(3) = HX8357_BLUE;
}
...
void setupDisplay()
{
  ...
  {
    timeUpdateScreen = 0;
    rectangle.fillScreen(index);
    displayDevice.writeRect8BPP(0, 0, rectangle.width(), rectangle.height(), rectangle.getBuffer(), colorPalette.data());
    displayDevice.updateScreen();
